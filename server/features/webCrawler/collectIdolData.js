...
async function crawlIdol(name) {
	...
	while (true) {
		...
		const fileHtmlPath = path.join(process.cwd(), "cached", name + "_" + pageCount + ".html")
		if (fs.existsSync(fileHtmlPath)) {
			console.log("File already exists:", fileHtmlPath);
			htmlContentRoot = fs.readFileSync(fileHtmlPath, "utf-8");
		} else {
			await axios(url, {
				"headers": {
					"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
				...
				.then(response => {
					...
					fs.writeFileSync(fileHtmlPath, htmlContentRoot);
				})
				.catch((err) => console.error(err));
		}
		// Get the root
	...
	const fileJsonPath = path.join(process.cwd(), "cached", name + ".json")
